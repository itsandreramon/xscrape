...
class XscrapeHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/health":
            ...
            # build command
            cmd = [
                "/app/.venv/bin/python", "-m", "xscrape.scraper",
                "--port", "9222",
                "--hours", hours,
                "--feed", feed,
            ...
            try:
                ...
                # copy updated package to /app
                import shutil
                files_updated = []
                src_pkg = Path("/app/repo/xscrape")
                dst_pkg = Path("/app/xscrape")
                if src_pkg.exists():
                    shutil.rmtree(dst_pkg, ignore_errors=True)
                    shutil.copytree(src_pkg, dst_pkg)
                    files_updated.append("xscrape/")

                # send response then restart
                self.send_json({
                    "status": "success",
                    "git_output": git_output,
            ...
            cmd = [
                "/app/.venv/bin/python", "-m", "xscrape.cookies",
                "--port", "9222",
